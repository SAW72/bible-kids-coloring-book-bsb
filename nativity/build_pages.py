...
def compose_cover() -> Path:
    ...
    draw = ImageDraw.Draw(page)
    y = 88
    title_face, stroke = fit_title_size(draw, "The Nativity", W - 220, 230)
    h = draw_centered(draw, "The Nativity", y, title_face, fill=255, stroke=stroke)
    y += h + 18
    sub = font(FRED_BOLD, 92)
    h = draw_centered(draw, "A Coloring Story Book", y, sub)
    y += h + 16
    brand = font(NUNITO_XB, 58)
    h = draw_centered(draw, "Steward of the King", y, brand)
    y += h + 12
    small = font(NUNITO, 36)
    h = draw_centered(draw, "Stories inspired by the Berean Standard Bible", y, small)
    y += h + 28

    art_bottom = FOOTER_INK_TOP - 56
    art = lineart(ART / "cover.png", W - 120, art_bottom - y)
    ax = (W - art.width) // 2
    ay = y + max(0, (art_bottom - y - art.height) // 2)
    page.paste(art, (ax, ay))
    draw_footer(draw)
...
def compose_parent_note() -> Path:
    page = Image.new("L", (W, H), 255)
    draw = ImageDraw.Draw(page)
    # Same frame as the volume parent notes: outer edge at 70, bottom at 3164, 8px stroke.
    frame(draw, 70, 70, 2479, 3164, 8)
    ...
    def line_height(face: ImageFont.FreeTypeFont, sample: str = "Ag") -> int:
        _, height, _ = text_size(draw, sample, face)
        return height

    def para(text: str, face: ImageFont.FreeTypeFont, leading: int, gap: int, indent: int = 0) -> None:
        nonlocal y
        for line in wrap(draw, text, face, width - indent):
            draw.text((left + indent, y), line, font=face, fill=0)
            y += leading
        y += gap
    ...
    def heading(text: str) -> None:
        ...
        draw.text((left, y), text, font=face, fill=0)
        y += line_height(face) + 16

    title = font(FRED_BOLD, 108)
    draw.text((left, y), "For Parents", font=title, fill=0)
    y += line_height(title, "For Parents") + 4
    draw.text((left, y), "& Sunday School", font=title, fill=0)
    y += line_height(title, "& Sunday School") + 28
    ...
    heading("The story in this book")
    note = font(NUNITO, 38)
    stories = [
    ...
    if y > 3080:
        raise SystemExit(f"Parent note overflowed the frame (y={y})")

    draw_footer(draw)
    out = PAGES / "14-parent-note.png"
    save_page(page, out)
    print(f"parent note ended at y={y}")
    return out
